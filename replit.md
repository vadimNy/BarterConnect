...
## Architecture
- `client/src/pages/` - All page components (landing, auth, dashboard, create-request, my-requests, matches, interests, messages, public-request)
- `client/src/lib/auth.tsx` - Auth context provider with login/signup/logout
- `client/src/components/app-layout.tsx` - Authenticated app layout with nav + avatar upload
...
- `server/routes.ts` - All API endpoints (auth, requests, matches, interests, conversations, public, uploads)
...
## Database Tables
- `users` - id (serial), email, password_hash, name, city, avatar_url, created_at
